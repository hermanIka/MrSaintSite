...
export function AdminVisaRequestsPage() {
  ...
  const { data: requests = [], isLoading } = useQuery<VisaRequest[]>({
    queryKey: ["/api/admin/visa-requests"],
    queryFn: async () => {
      const res = await fetch("/api/admin/visa-requests", { headers: getAuthHeaders() });
  ...
  const updateStatusMutation = useMutation({
    mutationFn: async ({ id, status, notes }: { id: string; status: string; notes: string }) => {
      const res = await fetch(`/api/admin/visa-requests/${id}/status`, {
        method: "PUT",
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/visa-requests"] });
  ...
  const handleOpen = (request: VisaRequest) => {
  ...
  return (
    <AdminLayout title="Demandes Visa">
      {/* Stats bar */}
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p>Aucune demande de visa{filterStatus !== "all" ? " avec ce statut" : ""}</p>
            </div>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b bg-muted/40">
                    ...
                    <th className="text-left px-4 py-3 font-medium text-muted-foreground hidden sm:table-cell">Type visa</th>
                ...
                <tbody className="divide-y divide-border">
                  {filtered.map((req) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 hidden md:table-cell text-foreground">{req.destination}</td>
                      <td className="px-4 py-3 hidden lg:table-cell">
                        <span className="text-foreground font-medium">{req.amount}€</span>
                        <span className="text-xs text-muted-foreground ml-1">
                          {req.paymentMethod === "pawapay" ? "Mobile Money" : req.paymentMethod === "maishapay" ? "Carte" : ""}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3 text-right">
                        <Button
                          size="icon"
                          variant="ghost"
                          data-testid={`button-view-visa-${req.id}`}
      ...
      <Dialog open={!!selectedRequest} onOpenChange={() => setSelectedRequest(null)}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              Demande de visa — {selectedRequest?.firstName} {selectedRequest?.lastName}
            </DialogTitle>
          </DialogHeader>
            ...
            <div className="space-y-6">
              ...
              <section>
                ...
                <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
                  {[
                    ...
                    ["Type de visa", VISA_TYPE_LABELS[selectedRequest.visaType] || selectedRequest.visaType],
                ...
                <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
                  ...
                  <div>
                    ...
                    <span className="ml-1 font-medium text-foreground">
                      {selectedRequest.paymentMethod === "maishapay" ? "Carte bancaire" : selectedRequest.paymentMethod === "pawapay" ? "Mobile Money" : selectedRequest.paymentMethod || "—"}
                    </span>
                  </div>
              ...
              <section>
                ...
                <div className="space-y-2">
                  {[
                    ["Passeport", selectedRequest.passportUrl],
                    ["Photo d'identité", selectedRequest.photoUrl],
                    ["Document justificatif", selectedRequest.supportingDocUrl],
                  ].map(([label, url]) => (
