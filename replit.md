...
- **Database:** PostgreSQL with Drizzle ORM, structured with tables for `trips`, `testimonials`, `portfolio`, `faqs`, `admins`, `activity_logs`, `credit_travel_requests`, `chatbot_conversations`, `chatbot_system_prompts`, `go_plus_plans`, `go_plus_cards`, `go_plus_transactions`, and `visa_requests`. Includes a seed script for initial data.
